...
@dataclass
class SecretStrConfig(EnvConfigBase):
...
def test_secretstr_default():
    ...
    config = SecretStrConfig.load_from_env()

    # Test that default value is properly cast to SecretStr
    assert isinstance(config.secret, SecretStr), (
        f"Expected SecretStr, got {type(config.secret)}"
    )
    assert config.secret.get_secret_value() == "abc", (
        f"Expected 'abc', got {config.secret.get_secret_value()}"
    )

    # Test that normal field remains as string
    assert isinstance(config.normal_field, str), (
        f"Expected str, got {type(config.normal_field)}"
    )
...
def test_secretstr_from_env():
    ...
    assert config.secret.get_secret_value() == "xyz", (
        f"Expected 'xyz', got {config.secret.get_secret_value()}"
    )
